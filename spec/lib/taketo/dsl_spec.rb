...
module Taketo
  ...
  describe DSL do
    ...
    describe "#include_shared_server_config" do
      ...
      context "when there are symbol arguments (names of shared configs) and a hash" do
      ...
      it "raises DSLConfigError if non-existent config included" do
        within_parent_dsl(:server) do |c|
          expect { c.include_shared_server_config(:foo) }.to raise_error(DSLConfigError, "Shared server config 'foo' is not defined!")
        end
      end
    end
