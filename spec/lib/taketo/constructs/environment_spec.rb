  ...
  module Constructs
    ...
    describe Environment do
      ...
      it "it holds RAILS_ENV default when assigned a default server config" do
        environment.default_server_config = ServerConfig.new { |c| c.add_environment_variables(:FOO => :bar) }
      ...
      it "does not overwrite assigned default server config's RAILS_ENV" do
        environment.default_server_config = ServerConfig.new { |c| c.add_environment_variables(:RAILS_ENV => "bar") }
